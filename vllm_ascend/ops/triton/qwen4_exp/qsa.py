...
def _store_qsa_kv_rows_kernel(
    ...
    key_mask = valid_slot & (row < num_key_updates) & (dims < WIDTH)

    value_mask = valid_slot & (row < num_value_updates) & (dims < WIDTH)
    ...
    tl.store(
        k_cache_ptr + block * stride_k_cache_block + offset * stride_k_cache_token + dims * stride_k_cache_dim,
        key,
        mask=key_mask,
    )
    value = tl.load(
    ...
    tl.store(
        v_cache_ptr + block * stride_v_cache_block + offset * stride_v_cache_token + dims * stride_v_cache_dim,
        value,
        mask=value_mask,
    )
...
def qsa_store_kv_cache_rows(
    ...
    if not HAS_TRITON or any(cache.device.type != "npu" for cache in caches):
        raise RuntimeError("QSA NPU cache stores require Triton")
    if any(cache.ndim != 4 or cache.shape[2] != 1 or not all(cache.shape) for cache in caches):
